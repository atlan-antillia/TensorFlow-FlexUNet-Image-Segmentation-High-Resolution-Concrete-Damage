...
class ImageMaskDatasetGenerator:
  ...
  def __init__(self, size=512, augmentation=True):
    ...
    self.RGB_COLORS = {
      "crack": (255, 0, 0),      #red
      "exposed rebar":(0,255,0), #green
      "corrosion":(0,255,255),   #cyan  
      "spalling":(128,128,128),  #gray
     }
  ...
  def generate(self, images_dir, masks_dir, output_images_dir, output_masks_dir):
    ...
    json_files  = sorted(glob.glob(masks_dir  + "/*.json"))
    num_images = len(image_files)
    num_jsons  = len(json_files)
    num = min(num_images, num_jsons)
    print(num_images, num_jsons)
    print("MIN {}".format(num))
    input("Hit any key")
    index = 10000
    for i in range(num):
      index += 1
      self.create_image_files(image_files[i], index, output_images_dir)
      self.create_mask_files(json_files[i], index, output_masks_dir)
      
  def create_image_files(self, image_file, index, output_images_dir):
  ...
  def create_mask_files(self, json_file, index, output_images_dir):
    ...
    with open(json_file, 'r') as f:     
      ...
      for shape in shapes:
        ...
        try:
          ...
          print("No found color to {}".format(label) ) 
          input("Error")
          continue
        ...
        print("Saved ", output_filepath)
        if self.augmentation:
            self.augment(mask,  filename , output_images_dir, border=(0, 0, 0), mask=False)
...
if __name__ == "__main__":
  try:
    ...
    # Merged image and annotations folder
    images_dir   = "./HRCDS-master/image/"
    masks_dir    = "./HRCDS-master/annotations/"   

    output_dir        = "./Augmented-HRCDS-master/"
    if os.path.exists(output_dir):
      shutil.rmtree(output_dir)
      os.makedirs(output_dir)

    output_images_dir = "./Augmented-HRCDS-master/images/"
    output_masks_dir  = "./Augmented-HRCDS-master/masks/"
